...
static zend_object*
php_g_hash_table_create_object(zend_class_entry *class_type)
{
    TRACE();
    php_g_hash_table *intern = zend_object_alloc(sizeof(php_g_hash_table), class_type);

    zend_object_std_init(&intern->std, class_type);
    object_properties_init(&intern->std, class_type);
    //g_print("php_g_hash_table_create_object\n");

    //php_g_hash_table_properties_init(intern);
    ZVAL_NULL(&intern->hash_func);
    ZVAL_NULL(&intern->key_equal_func);

    intern->ptr = NULL;// new GHashTable ?
    intern->properties = NULL;

    intern->std.handlers = &php_g_hash_table_handlers;

    //TRACE("php_g_hash_table_create_object(%p) / %d\n", &intern->std, intern->std.gc.refcount);
    return &intern->std;

}
/* }}} php_g_hash_table_create_object */
...
static void
php_g_hash_table_free_storage(zend_object *object)
{
    TRACE();
...
#if 0
        if (ZVAL_IS_php_g_hash_table(value)) {
            // do unset(object->next) and php_g_hash_table_insert(object, value, 0);
        } else {
...
/* {{{ php_g_hash_table_get_debug_info */
static HashTable*
php_g_hash_table_get_debug_info(zend_object *object, int *is_temp)
{
    php_g_hash_table *intern =  ZOBJ_TO_PHP_G_HASH_TABLE(object);
    HashTable   *debug_info,
                *std_props;
...
static int
php_g_hash_table_has_dimension(zval *object, zval *member, int check_empty)
{
    TRACE();
...
} /* }}} end php_g_hash_table_has_dimension */


static zval*
php_g_hash_table_read_dimension(zval *object, zval *offset, int type, zval *rv) /* {{{ */
{
    TRACE();
    zval offset_copy;
    if (!offset) {
...
static zval *php_g_hash_table_key_destroy = NULL;
static void
php_g_hash_table_key_destroy_func(gpointer data)
{
    TRACE();
    zval *val = (zval*)data;
    /*
    g_print("php_g_hash_table_key_destroy_func\n");
    g_print("    %p\n", data);
    ...
    */
    ...
    php_g_hash_table_key_destroy = data;
    Z_TRY_DELREF_P(val);
    efree(val);
}

static void
php_g_hash_table_value_destroy_func(gpointer data)
{
    TRACE();
    //g_print("php_g_hash_table_value_destroy_func\n");
    //g_print("    %p\n", data);
    //zval *val = (zval*)data;
    //Z_TRY_DELREF(*val);
    // add ref in php_g_hash_table_add
    if (php_g_hash_table_key_destroy==data) {
        //g_print("    value as key\n");
    }
    //Z_TRY_DELREF_P(val);

    php_g_hash_table_key_destroy = NULL;
}
...
php_g_hash_table_get_handlers()
{
    ...
    php_g_hash_table_handlers.offset = PHP_G_HASH_TABLE_OFFSET;
    ...
    php_g_hash_table_handlers.has_dimension = php_g_hash_table_has_dimension;
    php_g_hash_table_handlers.read_dimension = php_g_hash_table_read_dimension;

/*
    php_g_hash_table_handlers.unset_dimension;
    php_g_hash_table_handlers.write_dimension;
    php_g_hash_table_handlers.count_elements;
*/
...
php_g_hash_table_class_init(zend_class_entry *container_ce, zend_class_entry *parent_ce) {
   php_g_hash_table_get_handlers();
   PHP_GTK_INIT_CLASS_ENTRY((*container_ce), "GHashTable", php_g_hash_table_methods);
   php_g_hash_table_class_entry = zend_register_internal_class_ex(container_ce, parent_ce);
   php_g_hash_table_class_entry->create_object = php_g_hash_table_create_object;
   /*
...
php_g_hash_table_new(zval *hash_func, zval *key_equal_func)
{
    ...
    hash_table->ptr = g_hash_table_new_full(php_g_hash_table_hash_func,
                                            php_g_hash_table_key_equal_func,
                                            php_g_hash_table_key_destroy_func,
                                            php_g_hash_table_value_destroy_func);
